...
def solve(instance: Dict[str, Any], parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # === Added Validation for Transition Rules ===
    print("\n[Validation] Checking transition rules consistency...")
    for line, rules in transition_rules.items():
        if rules is None or not rules:
            print(f"⚠ Warning: No transition rules defined for line '{line}'. All transitions allowed.")
        else:
            for prev_grade, allowed_next in rules.items():
                if not allowed_next:
                    print(f"⚠ Warning: Grade '{prev_grade}' on line '{line}' has no allowed next transitions.")

    # === Robust Hard Constraints for Disallowed Transitions ===
    for line in lines:
