"""
CP-SAT Solver for production optimization (refactored)
- Objective in "Ensure All Grades' Production" mode now uses HORIZON (grade-level) normalization
  for stockout and minimum inventory deficit penalties, and total-demand normalization for
  closing inventory deficits, to stabilize penalty weights across days.

Original authorship preserved; structure cleaned up for readability.
"""
...
class SolutionCallback(cp_model.CpSolverSolutionCallback):
    ...
    def on_solution_callback(self):
        ...
        # Store production data per day (aggregated across lines)
        ...
        for grade in self.grades:
            ...
            # Final closing inventory separately
        ...
        # Store production schedule (which grade each line produces each day)
        for line in self.lines:
        ...
        # Objective breakdown placeholder (can be enhanced if needed)
    ...
    def calculate_objective_breakdown(self, solver_objective):
        """Calculate detailed breakdown of the objective value (placeholder)."""
        breakdown = {
            'stockout': 0,
            'transitions': 0,
...
def build_and_solve_model(
    ...
    # Create production variables (full capacity or zero per line/day)
    for grade in grades:
    ...
    # 3. Material running constraints
    ...
    for line in lines:
        if line in shutdown_periods and shutdown_periods[line]:
            ...
            # Pre-shutdown grade (day before the first shutdown day)
                    ...
                    if day_before >= 0:
                        ...
                        for other_grade in grades:
                            if other_grade != pre_grade and is_allowed_combination(other_grade, line):
                                ...
                                if other_var is not None:
                                    model.Add(other_var == 0)
            # Restart grade (day after the last shutdown day)
    ...
    for grade in grades:
        for d in range(num_days):
            ...
            stockout = model.NewIntVar(0, 100000, f'stockout_calc_{grade}_{d}')
    ...
    # Capacity utilization (non-shutdown days)
    if progress_callback:
    ...
    for line in lines:
        for d in range(num_days):
            if line in shutdown_periods and d in shutdown_periods[line]:
                continue
            production_vars_line_day = [
                get_production_var(grade, line, d)
                for grade in grades
                if is_allowed_combination(grade, line)
            ]
            if production_vars_line_day:
                model.Add(sum(production_vars_line_day) == capacities[line])

    # Run constraints (starts, min/max run lengths)
    if progress_callback:
        progress_callback(0.4, "Adding run constraints...")

    # 5a. Force start dates
    ...
    for grade in grades:
        for line in allowed_lines[grade]:
            ...
            # Define run_start as producing today and not producing yesterday
            for d in range(num_days):
                key = (grade, line, d)
                ...
                if line in material_running_info:
                    ...
                    has_forced_block = (
                        line in material_running_map and material_running_map[line]['expected_days'] > 1
                    )
    ...
    # Transition constraints (forbidden transitions)
    if progress_callback:
                    ...
                    if prev_grade in transition_rules[line] and is_allowed_combination(prev_grade, line):
                        ...
                        for current_grade in grades:
                            if (
                                ...
                                and current_grade not in allowed_next
                                and is_allowed_combination(current_grade, line)
                            ):
    ...
    for grade in grades:
        for line in allowed_lines[grade]:
            ...
            if not rerun_allowed.get(grade_plant_key, True):
                ...
                for d in range(num_days):
                    if line in material_running_map:
                        mat_grade = material_running_map[line]['material']
                        mat_days = material_running_map[line]['expected_days']
                        if mat_grade == grade and d < mat_days:
                            continue
                    key = (grade, line, d)
    ...
    # Soft constraints (inventory deficits and closing inventory)
    if progress_callback:
    ...
    for grade in grades:
        ...
        # Minimum closing inventory (SOFT) — evaluated at (num_days - buffer_days)
    ...
    if penalty_method == "Ensure All Grades' Production":
        # ENSURE ALL GRADES' PRODUCTION MODE with HORIZON normalization
        PERCENTAGE_PENALTY_WEIGHT = 100

        # Precompute horizon-level demand normalizers per grade
        grade_total_demand = {}
        for grade in grades:
            total = sum(demand_data[grade].get(dates[t], 0) for t in range(num_days))
            grade_total_demand[grade] = total

        # 1) Stockout penalties normalized by TOTAL horizon demand
        for grade in grades:
            normalizer = grade_total_demand[grade]
            if normalizer <= 0:
                normalizer = 1  # avoid division by zero for inactive grades
            penalty_per_unit_base = (PERCENTAGE_PENALTY_WEIGHT * stockout_penalty) / (normalizer + epsilon)
            scaled_base = int(penalty_per_unit_base * 100)
            for d in range(num_days):
                if (grade, d) in stockout_vars:
                    objective_terms.append(scaled_base * stockout_vars[(grade, d)])

        # 2) Minimum inventory deficit penalties normalized by TOTAL horizon demand
        ...
        for (grade, d), deficit_var in inventory_deficit_penalties.items():
            ...
            if normalizer <= 0:
                normalizer = 1
            penalty_coeff = (
                PERCENTAGE_PENALTY_WEIGHT * stockout_penalty * MIN_INV_VIOLATION_MULTIPLIER
            ) / (normalizer + epsilon)
            scaled_penalty = int(penalty_coeff * 100)
            objective_terms.append(scaled_penalty * deficit_var)

        # 3) Closing inventory deficit penalties normalized by TOTAL horizon demand
        for grade, closing_deficit_var in closing_inventory_deficit_penalties.items():
            normalizer = grade_total_demand[grade]
            if normalizer <= 0:
                normalizer = 1
            penalty_coeff = (PERCENTAGE_PENALTY_WEIGHT * stockout_penalty * 3) / (normalizer + epsilon)
            scaled_penalty = int(penalty_coeff * 100)
            ...
            for d in range(num_days - 1):
                for grade1 in grades:
                    ...
                    for grade2 in grades:
                        ...
                        model.Add(
                            trans_var >= is_producing[(grade1, line, d)] +
